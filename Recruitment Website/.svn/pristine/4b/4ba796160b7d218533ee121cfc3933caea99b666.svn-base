...
class WebShowController extends Controller implements Show{
	public function home(){
		R('WebShow/topJob_show');
		R('WebShow/comp_show');
		R('WebShow/newJob_sumVideoPeople_show');
		$this->display('HOME');
    }
    public function find_password(){
    	$uid = $_GET['params'];
    	$this->assign('uid',array('userid' => $uid ))->display();
    }
// 获取指定日期所在星期的开始时间与结束时间
	...
	public function newJob_sumVideoPeople_show(){
		...
		//$n = array('njc' => 'asd');
		//$v = array('vp' => $VideoPeople);
		$newJobCount = array('newjobc' => $newJobCount);
		$VideoPeople = array('videop' => $VideoPeople);
		...
		//dump($newJobCount);dump($VideoPeople);die;
	}

    //热门职位显示
	public function topJob_show(){
		//echo '12';
		$job_info=M('job_info , company_info , industry');
		$data=$job_info->where('company_info.hrId = job_info.hrid and job_info.industryId = industry.industryId')->order('jobClick desc')->limit(8)->select();  //选择排名前100的职位,如果需要还可添加分组
		$this->assign('topjobs',$data);  //分配到模板
		$this->assign('topjobs1',$data);
		$this->assign('hotjob',$data);
		//$hr_info=M('hr_info');
		//$hrs=$hr_info->where(array('hrId' => $jobs[0]['hrid']))->select();  //需要根据企业显示的信息定查询的字段
		//$this->assign('hrs',$hrs);  //分配到模
		//$num = count($jobs) - 1;
		//$i = 0;
		//$company_info=M('company_info');
		//echo $num;die
		//while($num>=$i){
		//	$companys[$i]=$company_info->where(array('hrId' => $jobs['hrid']))->select();  //需要根据企业显示的信息定查询的字段
		//	$i = $i + 1;
			//dump($companys);
		//}
	...
	public function comp_show(){
		$company_info=M('company_info , job_info');
		$companys=$company_info->where('company_info.hrId = job_info.hrid')->select();  //需要根据企业显示的信息定查询的字段
		$this->assign('companys',$companys);  //分配到模板
	...
	public function hr_interResume_show(){//本函数接口可用，已测试
			...
			$this->page = $page->show();// 分页显示输出
			$this->display('to_be_interviewed'); // 输出模板	
