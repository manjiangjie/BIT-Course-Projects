	...
	class HRController extends Users implements Register{
		...
		public function hrReg(){
			...
			$reg_data = array(
				...
				'userEmailCode' => mt_rand(100000,999999),
				'userTelCode' => mt_rand(100000,999999),
	        ...
	        $auto_rules=array(
		      array('userType','2'),  //HR注册将userType标志为1
	          //array('userEmail', 'unique', '邮箱名称已经存在！',1,'unique'),
	          //array(),  //自动填充当前时间戳
              //array('userPassword','md5',3,'function') , // 对userPassword字段在新增和编辑的时候使md5函数处理
              //array('userEmailCode' => mt_rand(100000,999999)),
              //array('userTelCode' => mt_rand(100000,999999)),
		    );
           ...
           // 验证通过 可以进行其他数据操作
	            ...
	            if($result){
               $data=array('hrId' => $result, 'hrEmail' => $reg_data['userEmail'], 'createTime' => $regTime);
			   $hr_info->data($data)->add();  //将该userId存入hrId
			   //$this->success('注册成功');
			    R('HR/emailCheck',array('emailAddr' => $reg_data['useremail']));
