...
$(function(){
		...
		$('#resetPwd_submit').click(function(){
			var password = $('input[name = reg_password]');
			var verify = $('input[name = verify_ip]');
		    ...
		    if(!password.val() || ! verify.val()){
				...
				//alert(uid[10]);
			$.post(reseturl,{"password" : password.val(),"verify" : verify.val(),"uid" : uid[10]},function (result){
						//alert(result.status);
						if(result.status == "验证码错误"){
							alert("验证码错误");
							changeVerify();
						}
						else if(result.status == "修改成功"){
							alert("修改成功");
							//fchangeVerify();
						}
						else if(result.status == "修改失败"){
							alert("修改失败");
							changeVerify();
						}
						/*else if(result.status == "用户不存在"){
							alert("用户不存在");
							changeVerify();
						}*/
						else{
		...
		$('#comfirm_btn').click(function (){
			var username = $('input[name = reg_password_comfirm]');
			var verify = $('input[name = verify_ip]');
			if(!username.val() || !verify.val()){
				alert("信息不能为空");
			}
			else{
				var uid_split = username.val().split("@");
				if(uid_split[1]){
					...
					if(uid_split1[1]){
						document.getElementById("fuidlabel1").style.visibility = "hidden";
						$.post(comfirmUrl, {"emailAddr" : username.val(),"verify" : verify.val()} , function (result){
							if(result.status == "验证码错误"){
								alert("验证码错误");
							}
							else if(result.status == "验证邮件已发出"){
								alert("验证邮件已发出");
							}
							else{
								alert("未响应");
