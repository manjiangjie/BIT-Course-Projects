...
// 获取指定日期所在星期的开始时间与结束时间
	...
	public function hr_recuitmentJob_show(){
		//说明：recuitmentJobNum为职务招聘数
            ...
            $limit ='3';
            $InterInfo=$M->where($sql)->order('interviewDeliverTime DESC')->limit($limit)->select();
            //dump($InterInfo);die;
            $this->assign('InterInfo',$InterInfo);// 赋值数据集
